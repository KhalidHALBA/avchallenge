...
void eventUpdate(ModelInstance *comp, fmiEventInfo *eventInfo)
{
	...
	// intialize buffers and delimiters.

	int init_size = strlen(buf);
	char delim[] = " ";
	char *ptr = strtok(buf, delim);

	// assign incoming and outgoing signals to the corresponding values.
	...
	// logging sent and received data at the FMU/IGNITE level.
	...
	// clearning the buufers and closing the sockets.

	fflush(stdout);
	memset(buf, 0, BUFLEN);
	memset(DYNAMICS, 0, BUFLEN);ss
	closesocket(s);
	WSACleanup();
